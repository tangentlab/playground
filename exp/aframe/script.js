...
const objectTypes = [
  ...
  { type: "tetrahedron", tag: "a-tetrahedron", props: 'radius="0.5"' },
  {
    type: "cone",
    tag: "a-cone",
    props: 'radius-bottom="0.5" radius-top="0.1" height="1"',
  },
...
function transformObject(element) {
  ...
  if (nextObject.tag === "a-box") {
    ...
    nextObject.tag === "a-octahedron" ||
    nextObject.tag === "a-tetrahedron" ||
    nextObject.tag === "a-dodecahedron"
  ) {
    newElement.setAttribute("radius", "0.5");
...
document.querySelector("a-scene").addEventListener("loaded", function () {
  ...
  const THREE =
    window.THREE ||
    (typeof AFRAME !== "undefined" && AFRAME.THREE ? AFRAME.THREE : null);
